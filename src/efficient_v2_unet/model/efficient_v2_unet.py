...
ENCODER_LAYER_NAMES = {
    ...
    # FIXME or should it be the normalization?
    ...
    # FIXME or should it be 'block2b_add'?
    "b0_s4": "block4a_expand_activation",  # 'Activation' layer with size 32
    # Activation layer with size 16 ('Bottleneck')
    "b0_b1": "block6a_expand_activation",  # Fixme, is this really the bottleneck?
    #  or another skip layer?
    # bottle neck ?=
    "b1_s1": "input_1",
...
def build_efficient_v2_unet(efficient_model, input_shape) -> Model:
    ...
    to convert the efficientNet to a UNET this video was helpful:
...
def get_callbacks(
    best_only="my_efficientUNet_model_best_ckp.h5",
    ...
    callbks = [
        callbacks.ModelCheckpoint(
            best_only, save_best_only=True, monitor=monitor, mode="max", verbose=1
        )
    ]
    if early_stop:
        callbks.append(callbacks.EarlyStopping(patience=7, monitor=monitor, verbose=1))
    if tensor_board_logdir is not None:
        callbks.append(
            callbacks.TensorBoard(log_dir=tensor_board_logdir, write_images=True)
        )
...
def create_and_train(
    ...
    if not os.path.exists(train_img_dir):
        raise OSError(f"The training image path does not exist: " f"<{train_img_dir}>")
    if not os.path.exists(train_mask_dir):
        raise OSError(f"The training mask path does not exist: " f"<{train_mask_dir}>")

    # Split input images into train, validation and test images     ----------
    if val_img_dir is not None:
        if not os.path.exists(val_img_dir):
            raise OSError(
                f"The validation image path does not exist: " f"<{val_img_dir}>"
            )
        if val_mask_dir is None or not os.path.exists(val_mask_dir):
            raise OSError(
                f"The validation mask is missing or wrong: " f"<{val_mask_dir}>"
            )
        ...
        if test_img_dir is None:
            ...
            if isinstance(test_mask_dir, str):
                print(
                    f"Warning: No test image path provided, but a test mask "
                    f"path <{test_mask_dir}>, which will be ignored."
                )
            (train_img_dir, train_mask_dir, _, _, test_img_dir, test_mask_dir) = (
                split_folder_files_to_train_val_test(
            ...
            if not os.path.exists(test_img_dir):
                raise OSError(
                    f"The test image path does not exist: " f"<{test_img_dir}>"
                )
            if not os.path.exists(test_mask_dir):
                raise OSError(
                    f"The test mask path does not exist: " f"<{test_mask_dir}>"
                )
        ...
        if test_img_dir is None:
            ...
            (train_img_dir, train_mask_dir, val_img_dir, val_mask_dir, _, _) = (
                split_folder_files_to_train_val_test(
    ...
    if n_train_img / n_total_img < 0.6:
        print(
            "!!! WARNING: you have less than 60% of training images. "
            "Consider aborting..."
        )
    if n_val_img / n_total_img < 0.1:
        if n_val_img == 0:
            ...
            print(
                "!!! WARNING: you have less than 10% of validation images. "
                "Consider aborting..."
            )
    if n_test_img / n_total_img < 0.1:
        if n_test_img == 0:
            print(
                "!!! WARNING: you have 0 test images. You won't be able to "
    ...
    if os.path.exists(path):
        print(
            f"Warning: the model already exists and will be overwritten "
            f"(in: {path})"
        )
    ...
    if train_gen.get_classes() > 1:
        raise NotImplementedError(
            "Training models for more than one class " "is not yet implemented."
        )
    ...
    model = build_efficient_v2_unet(
        efficient_model=efficientnet, input_shape=(None, None, 3)
    )
...
def evaluate_model(
    ...
    if not os.path.exists(img_dir_path):
        raise OSError(f"The image folder does not exist: <{img_dir_path}>")
    if not os.path.exists(mask_dir_path):
        raise OSError(f"The mask folder does not exist: <{mask_dir_path}>")
    img_paths = glob.glob(img_dir_path + "/*" + file_extension)
    if len(img_paths) == 0:
        raise RuntimeError(
            f"No image files ({file_extension}) found in: " f"{img_dir_path}"
        )
    mask_paths = []
    ...
    for i in range(1, 4):  # Resolutions: 1/1, 1/2, 1/3
        ...
        if os.path.exists(test_path):
            print(
                f"!!!Warning the output directory for the test predictions "
        ...
        for img_path, pred_img in zip(img_paths, predictions):
            ...
            if best:
                ...
                path = path.replace(file_extension, "_best-ckp" + file_extension)
        ...
        thresholds = [x / 10 for x in range(0, 10)]  # threshold of 1.0 is useless
        for t in thresholds:
            (acc, iou) = calc_metrics(y_true=gt, y_pred=predictions, threshold=t)
...
if __name__ == "__main__":
    dir_img_unsplit = (
        "G:/20231006_Martin/Plaque-Size-Samples_annotations_photoshop/all_images"
    )
    dir_mask_unsplit = (
        "G:/20231006_Martin/Plaque-Size-Samples_annotations_photoshop/all_masks"
    )
    all_train_img_split = (
        "G:/20231006_Martin/Plaque-Size-Samples_annotations_photoshop/all_images/train"
    )
    all_train_mask_split = (
        "G:/20231006_Martin/Plaque-Size-Samples_annotations_photoshop/all_masks/train"
    )
    all_val_img_split = (
        "G:/20231006_Martin/Plaque-Size-Samples_annotations_photoshop/all_images/val"
    )
    all_val_mask_split = (
        "G:/20231006_Martin/Plaque-Size-Samples_annotations_photoshop/all_masks/val"
    )
    all_test_img_split = (
        "G:/20231006_Martin/Plaque-Size-Samples_annotations_photoshop/all_images/test"
    )
    all_test_mask_split = (
        "G:/20231006_Martin/Plaque-Size-Samples_annotations_photoshop/all_masks/test"
    )
    model_type = "b0"
    ...
    # evaluate
    """
    # Old info (for testing).

    model_path = '../../../models_test/models/test_history_model/test_history_model.h5'
    model_path ='../../../models/my_efficientUNet-B3_allIMGs' \
                '/my_efficientUNet-B3_allIMGs_best-ckp.h5'
    ...
    best_model = 'G:/20231006_Martin/EfficientUNet/models/my_efficientUNet-B3_allIMGs' \
                 '/my_efficientUNet-B3_allIMGs.h5'
